...
test.describe('Malicious Buyer Traffic Simulation', () => {
  ...
  test('price manipulation attempt', async ({ page }) => {
    ...
    // Reload the page to verify DOM manipulation doesn't persist in app state
    // Prices are served from the backend; client-side DOM changes should not survive a reload
    await page.reload({ waitUntil: 'networkidle' });

    const pageContent = await page.textContent('body');
    expect(pageContent).not.toContain('$0.01'); // Manipulated price should not persist after reload
  });
});
